...
function SWBuildingPortrait13({kind,level=1,className='',palette='coastal'}){
 const ref=C.useRef(null);
 C.useEffect(()=>{let alive=true;Promise.all([_u('/art/sculpted-buildings-v13.png'),_u('/art/sculpted-utilities-v13.png')]).then(([buildings13,utilities13])=>{if(!alive||!ref.current)return;const canvas=ref.current,ctx=canvas.getContext('2d'),art=SW13BuildingArt({kind,level},{buildings13,utilities13});ctx.clearRect(0,0,256,230);if(!art)return;const {image,frame}=art,scale=Math.min(212/frame.w,200/frame.h);ctx.drawImage(image,frame.x,frame.y,frame.w,frame.h,(256-frame.w*scale)/2,216-frame.h*scale,frame.w*scale,frame.h*scale);SWColorBuildingTile(ctx,256,230,{palette});});return()=>{alive=false};},[kind,level,palette]);
 return swElement('canvas',{ref,width:256,height:230,className:'sw-building-portrait13 '+className,'aria-hidden':true});
}
...
function SWShipPortrait13({kind,level=1}){
 const ref=C.useRef(null);C.useEffect(()=>{let alive=true;_u('/art/ships-v13.png').then(ships13=>{if(!alive||!ref.current)return;const art=SW13ShipArt(kind,level,{ships13}),ctx=ref.current.getContext('2d');ctx.clearRect(0,0,240,220);if(!art)return;const [x,y,w,h]=art.rect,scale=Math.min(220/w,200/h);ctx.drawImage(art.image,x,y,w,h,(240-w*scale)/2,212-h*scale,w*scale,h*scale);});return()=>{alive=false};},[kind,level]);return swElement('span',{className:'ship-art sw-ship-portrait13'},swElement('canvas',{ref,width:240,height:220,'aria-hidden':true}));
}
