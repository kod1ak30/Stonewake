...
(() => {
  ...
  // frontend/runtime/battle-worker.js
  var workerScope = (
    /** @type {any} */
    globalThis
  );
  workerScope.onmessage = ({ data }) => {
    try {
      workerScope.postMessage({ id: data.id, result: Kl(data.input) });
    } catch (error) {
      workerScope.postMessage({ id: data.id, error: error instanceof Error ? error.message : "Battle simulation failed." });
    }
  };
