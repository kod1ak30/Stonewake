...
(function () {
  ...
  function preview(input, jobs, now) {
    ...
    for (const job of jobs) for (const command of job.commands) {
      ...
      if (command.type === 'deploy') (result.orders ||= []).push({kind:command.kind,x:command.x,y:command.y,time,...(command.shipId?{shipId:command.shipId}:{})});
      else if (command.type === 'ship') (result.shipOrders14 ||= []).push({...command.order,time});
