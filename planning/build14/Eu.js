function Eu({
  ...
  const navalPresent=!!navalSupport||!!p?.units?.some(unit=>unit.naval);
  if(_&&navalPresent)for(const tile of[{x:-5,y:0},{x:-5,y:3},{x:-3,y:3}]){const point=vu(tile.x,tile.y);townRects.push({left:point.x-48,right:point.x+48,top:point.y-100,bottom:point.y+18});}
  ...
  const swBattleBottom=p?(M.w<M.h?132:78):0;
  let Ne = _
    ...
    Pe = Ne * A,
    Fe = _ && p ? Math.max(0,(swBattleBottom-40)/2) : 0,
    ...
    let d = 0,
      ...
      D = (r) => {
        ...
        (o.save(),
          ...
          (o.lineJoin = `round`),
          (o.lineCap = `round`));
        if(k.current.worldLand)SWDrawWorldTerrain(o,k.current.worldLand,{left:cameraX+(-M.w/2-Re)/Pe,top:cameraY+(-M.h/2-ze+Fe)/Pe,right:cameraX+(M.w/2-Re)/Pe,bottom:cameraY+(M.h/2-ze+Fe)/Pe});
        ...
        if(navalSupport&&!navalUnits.length&&!a?.units.some(unit=>unit.id===`naval-`+navalSupport.id)){
        ...
        if (T && (swDeploymentState.current.reserveCount===undefined||swDeploymentState.current.reserveCount>0)) SWDrawDeploymentEdge(o,Ue,Pe,swPreview.current);
    ...
    children: [
      ...
      (0, F.jsx)(`canvas`, {
        ...
        onPointerDown: (event) => {
          ...
          const world=We(event.clientX,event.clientY),tile=yu(world.x,world.y),target=T?SWDeploymentTarget(Ue,world,Pe):null;
        ...
        onPointerUp: (event) => {
          ...
          if(!gesture.multi&&me.current){
            if(gesture.mode.endsWith(`deploy`)){
              if(me.current.moved){
                const path=gesture.path.filter((target,index,path)=>(index===0||target.x!==path[index-1].x||target.y!==path[index-1].y)&&!gesture.deployedTiles?.has(`${target.x},${target.y}`)).slice(0,8);
                for(const target of path)if(!swFireDeployment(target,false))break;
